...
<body class="bg-gradient-to-b from-[#89CFF0] to-white min-h-screen flex items-center justify-center relative">
    ...
    <div
        ...
        <form action="/quiz/store" method="POST" class="w-full">
            <!-- Quiz selection -->
            <label for="quiz_selection" class="block text-gray-800 font-semibold mb-4 text-lg"
                style="font-family: 'Poppins', sans-serif;">Select a Quiz:</label>
            <select name="quiz_selection" id="quiz_selection"
                class="w-full p-4 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-300 ease-in-out text-lg mb-6">
                <option value="">Choose a quiz...</option>
                <?php foreach ($quizzes as $quiz): ?>
                    <option value="<?= htmlspecialchars($quiz['quiz_id']) ?>"><?= htmlspecialchars($quiz['title']) ?>
                    </option>
                <?php endforeach; ?>
