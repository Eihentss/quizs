        ...
        <?php if (isset($quizzes) && !empty($quizzes)): ?>
            <div class="flex justify-center items-center">
                <form method="GET" action="/quiz/start"
                    class="bg-white/70 p-8 rounded-lg shadow-lg w-full max-w-md backdrop-filter backdrop-blur-md border border-gray-200">
                    <label for="quiz-select" class="block text-gray-800 font-medium mb-3"
                        style="font-family: 'Poppins', sans-serif;">Choose a quiz:</label>
                    <select id="quiz-select" name="quiz_id"
                        class="w-full p-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-300 ease-in-out">
                    ...
                    <button type="submit"
                        class="mt-6 w-full bg-white text-black font-bold py-3 px-6 rounded-lg shadow-2xl transition-transform duration-300 ease-in-out hover:scale-105 hover:shadow-3xl hover:bg-gray-200">
                        Start Quiz
                    </button>
                </form>
            </div>
